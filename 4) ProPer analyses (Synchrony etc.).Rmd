...
```{r HAUKE_change_inf_into_v}

# change "inf" to "v" in syll_label and focus
main_df <- mutate(main_df, syll_label = ifelse(syll_label == "inf-p", "v-p", syll_label))
main_df <- mutate(main_df, syll_label = ifelse(syll_label == "inf-u", "v-u", syll_label))

main_df <- mutate(main_df, focus = ifelse(focus == "(inf)", "(v)", focus))
main_df <- mutate(main_df, focus = ifelse(focus == "inf", "v", focus))
...
if(useManual==TRUE) comp_df <- mutate(
  group_by(comp_df, file),
  # stretch auto_bounds observations to locate missing auto_bounds
...
# detect_sel_file <- "a020_a_m_interviewer_dir_nc_noa_rep_exp" #files[4] # or: "filename"
####################################
...
##--save?
ggsave(singleBounds, file=paste0("plots/",detect_sel_file,"_autoMan",autoMan,"_autoBoundsPlot.pdf"),device=cairo_pdf)
...
```{r}
#### self coded ####
# get absolute interval duration #

# abgeschaut von unten (*run fun*)
...
# #
# 
# intervals$dur <- intervals$end - intervals$start
# #
# 
# 
# #
# labels <- data.frame(
#   file = comp_df$file,
#   label = comp_df$syll_label
#   )
# labels <- subset(labels, !is.na(labels$label))
# labels <- subset(labels, labels$label != "")
# 
# labels$start <- intervals$start
# labels$end <- intervals$end
# labels <- mutate(labels, intervalLength = end - start)
...
# ####################################
# ##### manual singles, no-loop ######
# sel_file <- "a020_a_m_interviewer_dir_nc_noa_rep_exp" #files[3] # or: "filename"
# ####################################
