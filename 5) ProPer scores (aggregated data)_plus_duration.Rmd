...
scores_df <- mutate (
  scores_df, syll_start = na.locf(syll_start, na.rm=F)
)
scores_df <- mutate (
  scores_df, syll_end = na.locf(syll_end, na.rm=F)
)
...
### ##
# create autoSyll_start und autoSyll_ende, die erst mal dieselben Werte haben wie auto_bounds
scores_df <- mutate(
  scores_df, autoSyll_start = auto_bounds
)
scores_df <- mutate(
  scores_df, autoSyll_end = auto_bounds
)

#Plan:
#Füll NAs nach unten auf für autoSyll_start
#Füll NAs nach oben auf für autoSyll_end
scores_df <- scores_df %>%
  group_by(speaker) %>%
  fill(autoSyll_start, .direction = "down") %>%
  fill(autoSyll_end, .direction = "up")
###
...
## reduce columns
mini_scores_df <- droplevels(subset(scores_df, select = -c(t,
                                                           # syll_start, syll_mid, syll_end,
                                                           syll_bounds,
                                                           # word_start, word_mid, word_end, word_bounds,
                                                           perFloorStatus, relToStatus,  strengThreshStatus, f0_data_min, f0_data_max, f0_data_range, smogPP_20Hz, smogPP_12Hz, smogPP_8Hz, smogPP_5Hz, f0_interp_stretch_smooth, f0_interp_smooth, f0_realFloorStretch, syll_boundsSeq, useManualStatus, autoManStatus, averageSyllStatus,
                                                           # auto_bounds, auto_boundsSeq, f0atCoG,
                                                           PERatCoM, PERatCoG, CoM_corr, CoG_corr, CoMatCoM, syllCoMMap, syllCoMMap2, syllCoMflicts)))
